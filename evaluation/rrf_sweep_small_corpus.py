"""
RRF parameter sweep for small corpus (1800-1810) at fixed fetch_size.
Tests whether BM25-alone still beats Hybrid across parameter range on restricted corpus.
"""
...
# Small corpus qrels (same queries, but we'll filter results to 1800-1810)
...
# Year range for small corpus
SMALL_CORPUS_START = 1800
SMALL_CORPUS_END = 1810

def filter_by_year(candidates, year_start, year_end):
    """Filter candidates to only include docs within year range."""
    filtered = []
    for r in candidates:
        yr = r.get('publication_year')
        if yr is not None and year_start <= yr < year_end:
            filtered.append(r)
    return filtered
...
print(f"RRF Parameter Sweep - Small Corpus (1800-1810) (fetch_size={FETCH_SIZE})")
print(f"Queries: {len(qrels)}")
print(f"k values: {k_values}")
print(f"alpha values: {alpha_values}")
print()

# Pre-fetch BM25 and Dense candidates for each query (full corpus, then filter)
print("Pre-fetching BM25 and Dense candidates...")
bm25_cache = {}
dense_cache = {}
for q in qrels:
    query = q['query_text']
    bm25_all = search_bm25(query, size=FETCH_SIZE)
    dense_all = search_dense(query, size=FETCH_SIZE)
    
    # Filter to small corpus years
    bm25_cache[query] = filter_by_year(bm25_all, SMALL_CORPUS_START, SMALL_CORPUS_END)
    dense_cache[query] = filter_by_year(dense_all, SMALL_CORPUS_START, SMALL_CORPUS_END)

# Also get pure BM25 baseline (filtered)
print("Evaluating pure BM25 baseline (small corpus)...")
bm25_results = []
for q in qrels:
...
out_path = os.path.join(os.path.dirname(__file__), 'rrf_sweep_small_corpus_results.json')
